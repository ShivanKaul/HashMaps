...
test('other "to"-containing place names stay single searches', () => {
  assert.deepEqual(parseInput('shibuya tokyo', '', ''), [
    'shibuya%20tokyo',
    '',
  ]);
...
test('"X to Y" is parsed as directions', () => {
  assert.deepEqual(parseInput('montreal to toronto', '', ''), [
    'montreal',
    'toronto',
  ]);
});
...
test('home/work keywords resolve to saved addresses', () => {
  assert.deepEqual(parseInput('home to work', '123 Main St', '456 King St'), [
    '123%20Main%20St',
    '456%20King%20St',
  ]);
});
...
test('the destination keyword resolves to a saved address', () => {
  assert.deepEqual(parseInput('office to home', '123 Main St', ''), [
    'office',
    '123%20Main%20St',
  ]);
});
...
test('special characters in addresses are URI-encoded', () => {
  assert.deepEqual(parseInput('home', '1 King St W, Toronto', ''), [
